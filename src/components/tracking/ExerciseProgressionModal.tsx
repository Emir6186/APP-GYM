...
import { Dumbbell, TrendingUp, Eye } from 'lucide-react';
...
export const ExerciseProgressionModal: React.FC<ExerciseProgressionModalProps> = ({
  ...
  sortedSessions.forEach(session => {
    const match = session.exercises?.find(e => e && e.exerciseName && e.exerciseName.toLowerCase() === exerciseName.toLowerCase());
    if (match) {
      ...
      (match.sets || []).forEach(s => {
        if (s && (s.isCompleted || s.weightKg > 0)) {
          exerciseVolume += ((s.weightKg || 0) * (s.reps || 0));
          completedSetsCount++;

          const oneRm = estimateOneRepMax(s.weightKg || 0, s.reps || 0);
      ...
      if (completedSetsCount > 0) {
        logs.push({
          ...
          sessionName: session.name || 'Entrenamiento'
        });
      }
    }
  });
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-slate-950 p-4 border border-slate-800 shadow-xl space-y-3">
            <div className="flex items-center space-x-3">
              {/* Foto de la máquina con zoom */}
              <div 
                onClick={() => {
                  if (photoUrl) setIsZoomOpen(true);
                }}
                className={`w-14 h-14 rounded-2xl bg-slate-950 border border-slate-700 overflow-hidden flex items-center justify-center text-emerald-400 font-bold text-lg flex-shrink-0 relative group/photo ${
                  photoUrl ? 'cursor-pointer' : ''
                }`}
                title={photoUrl ? "Tocar para ampliar foto" : undefined}
              >
                {photoUrl ? (
                  <>
            ...
            {logs.length === 0 ? (
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                {[...logs].reverse().map((log, idx) => {
                  ...
                  const dateFormatted = log.date ? new Date(log.date).toLocaleDateString('es-ES', {
                    weekday: 'short',
                    day: 'numeric',
                    month: 'short'
                  }) : 'Fecha reciente';
