...
import { Flame, Activity, Droplets, Apple, Sparkles, Check } from 'lucide-react';
...
export const ProfileTdeeCalculator: React.FC<ProfileTdeeCalculatorProps> = ({ onPlanRegenerated }) => {
  const { profile, updateProfile, metrics } = useTracking();
    ...
    <div className="space-y-6 pb-24">
      ...
      {showSavedToast && (
        <div className="fixed top-16 left-4 right-4 z-50 max-w-md mx-auto p-3 rounded-2xl bg-emerald-500 text-slate-950 font-bold text-sm shadow-2xl flex items-center justify-center gap-2 animate-bounce">
          ...
          ¡Métricas y Plan de Comidas actualizados con éxito!
        </div>
      )}

      {/* Tarjeta Resumen de Resultados Metabólicos */}
      <div className="rounded-3xl bg-gradient-to-br from-slate-900 via-slate-900 to-slate-950 border border-slate-800 p-5 shadow-2xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-200">Objetivo Calórico Diario</h3>
              <p className="text-[11px] text-slate-400">Fórmula Mifflin-St Jeor + Factor PAL</p>
            </div>
          </div>

          <div className="text-right">
            <span className="text-2xl font-black text-emerald-400 font-mono-numbers tracking-tight">
        ...
        {/* Comparativa TMB vs TDEE vs Target */}
        <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-800">
          <div className="p-2.5 rounded-xl bg-slate-950/60 border border-slate-800 text-center">
            <span className="text-[10px] text-slate-400 uppercase font-semibold block">TMB (Basal)</span>
            <span className="text-sm font-bold font-mono-numbers text-slate-200">{metrics.bmr}</span>
            <span className="text-[9px] text-slate-500 block">kcal en reposo</span>
          </div>

          <div className="p-2.5 rounded-xl bg-slate-950/60 border border-slate-800 text-center">
            <span className="text-[10px] text-slate-400 uppercase font-semibold block">Mantenimiento</span>
            <span className="text-sm font-bold font-mono-numbers text-blue-400">{metrics.tdee}</span>
            <span className="text-[9px] text-slate-500 block">Gasto Total</span>
          </div>

          <div className="p-2.5 rounded-xl bg-slate-950/60 border border-slate-800 text-center">
            <span className="text-[10px] text-slate-400 uppercase font-semibold block">Ajuste</span>
            <span className={`text-sm font-bold font-mono-numbers ${metrics.calorieDiff < 0 ? 'text-rose-400' : metrics.calorieDiff > 0 ? 'text-amber-400' : 'text-slate-300'}`}>
              {metrics.calorieDiff > 0 ? `+${metrics.calorieDiff}` : metrics.calorieDiff}
            </span>
            <span className="text-[9px] text-slate-500 block">kcal/día</span>
          </div>
        </div>

        {/* Desglose de Macronutrientes Óptimos */}
        <div className="pt-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            {/* Proteínas */}
            <div className="p-2.5 rounded-xl bg-slate-950/80 border border-rose-500/20">
              <span className="text-[10px] font-bold text-rose-400 block uppercase">Proteínas</span>
              <div className="text-sm font-black font-mono-numbers text-slate-100 mt-0.5">
                {metrics.proteinGrams} <span className="text-[10px] font-normal text-slate-400">g</span>
              </div>
              <span className="text-[9px] text-slate-500">{metrics.proteinGrams * 4} kcal (~2.2g/kg)</span>
            </div>

            {/* Carbohidratos */}
            <div className="p-2.5 rounded-xl bg-slate-950/80 border border-amber-500/20">
              <span className="text-[10px] font-bold text-amber-400 block uppercase">Carbohidratos</span>
              <div className="text-sm font-black font-mono-numbers text-slate-100 mt-0.5">
                {metrics.carbsGrams} <span className="text-[10px] font-normal text-slate-400">g</span>
              </div>
              <span className="text-[9px] text-slate-500">{metrics.carbsGrams * 4} kcal</span>
            </div>

            {/* Grasas */}
            <div className="p-2.5 rounded-xl bg-slate-950/80 border border-blue-500/20">
              ...
              <span className="text-[9px] text-slate-500">{metrics.fatGrams * 9} kcal (~0.9g/kg)</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-400 mt-2 px-1">
            <span className="flex items-center gap-1">
              ...
              Agua: <strong className="text-slate-200">{metrics.waterLiters} L/día</strong>
            </span>
            <span className="flex items-center gap-1">
              <Apple className="w-3.5 h-3.5 text-emerald-400" />
              Fibra: <strong className="text-slate-200">{metrics.fiberGrams} g/día</strong>
            </span>
          </div>
        </div>
      </div>

      {/* Formulario de Ajuste de Datos Biométricos */}
      <form onSubmit={handleSave} className="rounded-3xl bg-slate-900 border border-slate-800 p-5 space-y-4 shadow-xl">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div>
            <label className="block text-[11px] font-semibold text-slate-300 mb-1">Edad (años)</label>
            <input
              type="number"
              min="14"
              max="100"
          ...
          <div>
            <label className="block text-[11px] font-semibold text-slate-300 mb-1">Peso (kg)</label>
            <input
              type="number"
              step="0.1"
              min="30"
          ...
          <div>
            <label className="block text-[11px] font-semibold text-slate-300 mb-1">Altura (cm)</label>
            <input
              type="number"
              min="100"
              max="240"
        ...
        {/* Selección de Objetivo (Déficit vs Superávit vs Mantenimiento) */}
        <div>
        ...
        >
          ...
          Guardar y Recalcular Dieta Semanal
        </button>
      </form>
    </div>
  );
